...
//get the file
void server_accept(char *filename, client *tabclient){
...
int isend(char *msg, client *tabclient, int conex, int i){
  ...
  char *user;
  user=malloc(sizeof(char)*36);
  user=read_name(msg, send);
  char *filename;
  filename=malloc(sizeof(char)*36);
  filename=read_name(msg, concat_string(send, user));
  ...
  if (strcmp(command,"/msg") == 0 ){
      ...
      ask = concat_string(tabclient[i].name, " wants you to accept the transfer of the file named ");
